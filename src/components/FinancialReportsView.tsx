...
export default function FinancialReportsView() {
    ...
    const rows: TrialBalanceRow[] = (accs || []).map(acc => {
      ...
      items?.forEach(item => {
        if (item.account_id === acc.id) {
          ...
          const entryRelation = item.journal_entries as unknown as { date: string } | Array<{ date: string }>;
          const entryDate = Array.isArray(entryRelation) ? entryRelation[0]?.date : entryRelation?.date;
  ...
  async function queryBalanceSheet() {
    ...
    if (!plErr && plItems) {
      ...
      plItems.forEach(item => {
        const deb = Number(item.debit) || 0;
        const cred = Number(item.credit) || 0;
        const accountRelation = item.accounts as unknown as { type: string } | Array<{ type: string }>;
        const accountType = Array.isArray(accountRelation) ? accountRelation[0]?.type : accountRelation?.type;
  ...
  async function queryGeneralLedger() {
    ...
    const rows = (items || []).map(item => {
      ...
      const entryRelation = item.journal_entries as unknown as { date: string; entry_number: number; reference: string | null } | Array<{ date: string; entry_number: number; reference: string | null }>;
      const entry = Array.isArray(entryRelation) ? entryRelation[0] : entryRelation;
