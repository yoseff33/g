...
export default function SettingsView() {
  ...
  return (
    // إضافة w-full max-w-full min-w-0 لمنع التمرير الأفقي و overflow-x-hidden كحماية
    <div className="w-full max-w-full min-w-0 overflow-x-hidden space-y-6" id="settings_view">
      <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        {/* Core settings form */}
        <div className="bg-white border border-slate-100 p-4 sm:p-6 rounded-2xl shadow-sm lg:col-span-2 space-y-6">
          ...
          <form onSubmit={handleSaveSettings} className="space-y-4">
            ...
            {/* تحسين زر الحفظ ليكون كامل العرض على الجوال ومحاذي لليمين على الشاشات الأكبر */}
            <div className="flex flex-col sm:flex-row justify-end items-stretch sm:items-center gap-2 pt-2">
              <button 
                type="submit"
                className="w-full sm:w-auto bg-slate-800 hover:bg-slate-700 text-white font-medium px-5 py-2 rounded-lg text-sm flex items-center justify-center gap-2 transition-colors"
              >
        ...
        <div className="bg-white border border-slate-100 p-4 sm:p-6 rounded-2xl shadow-sm space-y-6">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              {/* تعديل الكود: إزالة truncate وإضافة overflow-x-auto مع break-all و max-w-full */}
              <code className="text-[10px] text-slate-600 font-mono bg-slate-50 p-2 border border-slate-200 rounded block overflow-x-auto break-all max-w-full">
                ...
                <div className="space-y-2">
                  <div className="flex items-start gap-2 text-red-800 text-xs font-bold bg-red-50 px-3 py-2 rounded-lg border border-red-100">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-red-600 font-normal mt-1 leading-normal font-mono break-words">{errorMessage}</p>
                    </div>
                  </div>
                  <div className="p-3 bg-amber-50 text-amber-900 border border-amber-200 text-xs rounded-lg flex items-start gap-1">
                    ...
                    <p className="text-[10px] leading-relaxed">تنبيه: لتفعيل النظام، قم بنسخ ملف <code className="bg-amber-100 px-1 rounded">.env.example</code> إلى <code className="bg-amber-100 px-1 rounded">.env</code> واملأ المتغيرات بالمفاتيح الصحيحة ثم أعد التشغيل.</p>
                  </div>
                </div>
              )}
            </div>
