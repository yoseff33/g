...
import { 
  Search, Printer, Download, RotateCcw, Eye, Edit, FileText, MessageCircle,
  BarChart3, Users, DollarSign, Calendar, Clock, CheckCircle, AlertCircle, XCircle
} from 'lucide-react'
...
type ReportFilters = {
  ...
  // تم إزالة date_from و date_to لعدم وجودهما في الـ View
}
...
export default function CustomReports() {
  ...
  async function fetchInitialData() {
    setLoading(true)
    try {
      // جلب العقود من الـ view، نستخدم serial_number للترتيب (موجود بالتأكيد)
      const { data: contracts, error } = await supabase
        .from('vw_contracts_report')
        .select('*')
        .order('serial_number', { ascending: false })  // الأحدث حسب الرقم التسلسلي (افتراضي)
      
      if (error) throw error
      
      // جلب المستثمرين والمحاسبين
      ...
      // جلب الأقساط لكل عقد لحساب الحالات
  ...
  function applyFiltersAndSort() {
    ...
    // فرز - باستخدام serial_number أو الخيارات الأخرى
    switch (sortBy) {
      case 'newest':
        data.sort((a, b) => b.serial_number - a.serial_number) // الأحدث = رقم أكبر
        break
      case 'oldest':
  ...
  return (
    <div className="p-4" dir="rtl">
      ...
      {/* الفلاتر والفرز - تم إزالة حقلي التاريخ */}
      <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 mb-6 no-print">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <FilterInput 
            label="اسم العميل" 
            value={filters.customer_name} 
            onChange={v => setFilters({...filters, customer_name: v})}
            placeholder="بحث بالاسم"
          />
          <FilterInput 
            label="رقم العقد" 
            value={filters.contract_number} 
            onChange={v => setFilters({...filters, contract_number: v})}
            placeholder="رقم العقد"
          />
          <FilterInput 
            label="رقم الهوية" 
            value={filters.national_id} 
            onChange={v => setFilters({...filters, national_id: v})}
            placeholder="الهوية"
          />
          <FilterSelect 
            label="المستثمر" 
            value={filters.investor_id} 
            onChange={v => setFilters({...filters, investor_id: v})}
            options={investors.map(i => ({ value: i.id, label: i.name }))}
            placeholder="جميع المستثمرين"
          />
          <FilterSelect 
            label="المحاسب" 
            value={filters.accountant_id} 
            onChange={v => setFilters({...filters, accountant_id: v})}
            options={accountants.map(a => ({ value: a.id, label: a.full_name || a.email }))}
            placeholder="جميع المحاسبين"
          />
          <FilterSelect 
            label="شركة التمويل" 
            value={filters.finance_company} 
            onChange={v => setFilters({...filters, finance_company: v})}
            options={[
              { value: 'إمكان', label: 'إمكان' },
              { value: 'تمارا', label: 'تمارا' },
              { value: 'تابي', label: 'تابي' },
              { value: 'مورا', label: 'مورا' },
              { value: 'كوارا', label: 'كوارا' },
              { value: 'تمام', label: 'تمام' }
            ]}
          ...
          <FilterSelect 
            ...
            onChange={v => setFilters({...filters, sale_type: v})}
            options={[
              { value: 'deferred', label: 'بيع آجل' },
          ...
          <FilterSelect 
            ...
            onChange={v => setFilters({...filters, client_status: v})}
            options={[
              { value: 'غير مستحق', label: 'غير مستحق' },
              { value: 'مستحق اليوم', label: 'مستحق اليوم' },
              { value: 'مستحق غداً', label: 'مستحق غداً' },
              { value: 'متأخر', label: 'متأخر' },
              { value: 'مكتمل السداد', label: 'مكتمل' }
            ]}
            placeholder="الكل"
          />
          <div className="flex items-end">
            <FilterSelect 
              ...
              options={[
                ...
                { value: 'highest_value', label: 'الأعلى قيمة' },
                { value: 'lowest_value', label: 'الأقل قيمة' },
                { value: 'nearest_due', label: 'الأقرب استحقاقاً' },
                { value: 'most_late', label: 'الأكثر تأخيراً' },
                { value: 'by_investor', label: 'حسب المستثمر' },
                { value: 'by_accountant', label: 'حسب المحاسب' }
              ]}
            />
          </div>
        </div>
        ...
        <table className="w-full text-sm text-center">
          ...
          <tbody className="divide-y divide-slate-800">
            {filteredData.length === 0 ? (
              <tr>
                <td colSpan={13} className="p-8 text-slate-500">لا توجد نتائج تطابق الفلاتر</td>
              </tr>
            ) : (
      ...
      <div className="mt-4 text-xs text-slate-500">
        إجمالي النتائج: {filteredData.length} عقد
      </div>
    </div>
  )
}

// مكونات مساعدة (نفسها كما كانت)
